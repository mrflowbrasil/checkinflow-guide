...
BEGIN
  INSERT INTO public.property_pages (property_id, page_key, title, icon, position) VALUES
    ...
    (NEW.id, 'nearby_services', 'Serviços Próximos', 'ConciergeBell', 25),
    (NEW.id, 'events_calendar', 'Calendário de Eventos', 'CalendarDays', 26);
  RETURN NEW;
END;
$function$;
...
-- Backfill new pages for existing properties (disabled by default)
INSERT INTO public.property_pages (property_id, page_key, title, icon, position, is_enabled)
SELECT p.id, 'nearby_services', 'Serviços Próximos', 'ConciergeBell', 25, false
FROM public.properties p
WHERE NOT EXISTS (
  SELECT 1 FROM public.property_pages pp
  WHERE pp.property_id = p.id AND pp.page_key = 'nearby_services'
);

INSERT INTO public.property_pages (property_id, page_key, title, icon, position, is_enabled)
SELECT p.id, 'events_calendar', 'Calendário de Eventos', 'CalendarDays', 26, false
FROM public.properties p
WHERE NOT EXISTS (
  SELECT 1 FROM public.property_pages pp
  WHERE pp.property_id = p.id AND pp.page_key = 'events_calendar'
);
