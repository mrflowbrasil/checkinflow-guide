-- Update seed function to include 'Senha Fechadura' at position 2
...
BEGIN
  INSERT INTO public.property_pages (property_id, page_key, title, icon, position) VALUES
    ...
    (NEW.id, 'review', 'Avaliação', 'Star', 21),
    (NEW.id, 'faq', 'FAQ', 'HelpCircle', 22);
  RETURN NEW;
END;
$$;

-- Shift existing pages down by 1 starting from position 2 to open a slot
UPDATE public.property_pages
SET position = position + 1
WHERE position >= 2
  AND page_key <> 'lock_code';

-- Backfill 'lock_code' for all existing properties at position 2
INSERT INTO public.property_pages (property_id, page_key, title, icon, position, is_enabled)
SELECT p.id, 'lock_code', 'Senha Fechadura', 'KeyRound', 2, true
FROM public.properties p
WHERE NOT EXISTS (
  SELECT 1 FROM public.property_pages pp
  WHERE pp.property_id = p.id AND pp.page_key = 'lock_code'
);