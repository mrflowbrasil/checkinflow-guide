    ...
    queryFn: async () => {
      const { data: property, error } = await supabase
        ...
        .select(`
          ...
          tenants!inner(id, name, primary_color, secondary_color, template, is_active),
  ...
  return (
    <div className={`guide-root guide-template-${template} min-h-screen`}>
      ...
      <Sheet open={!!activePageKey} onOpenChange={(o) => !o && setActivePageKey(null)}>
        <SheetContent side="bottom" className="h-[92vh] p-0 rounded-t-3xl border-t-0 max-w-md mx-auto" hideClose>
          {activePage && <PageContent pageId={activePage.id} title={activePage.title} template={template} onClose={() => setActivePageKey(null)} />}
        </SheetContent>
      </Sheet>
    </div>
  );
